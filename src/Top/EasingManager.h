...
public:
	...
	enum EasType {
		...
		Return,EasingTypeMax
	};
	static std::function<float(float, float, float)> getEas[];
	static void tCount(float& t,float speed);
	static bool tCountEnd(float t, bool nomal = true);
};